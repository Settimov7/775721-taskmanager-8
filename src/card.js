...
const createHashtags = (tags) => {
  ...
  tags.forEach((tag) => {
    hashtags += createHashtag(tag);
  });

  return hashtags;
};

const isDeadline = (date) => date < new Date();

const createCard = ({color = Default.COLOR, repeatingDays, title, dueDate, tags, picture}) => {

  const date = new Date(dueDate);
  const day = `${ date.toLocaleString(`en-US`, {day: `numeric`}) } ${ date.toLocaleString(`en-US`, {month: `long`}) }`;
  const time = `${ date.toLocaleString(`en-US`, {hour: `numeric`, minute: `numeric`}) }`;

  const cardClass = `card card--${ color } ${ isRepeat(repeatingDays) ? `card--repeat` : `` } ${ isDeadline(date) ? `card--deadline` : ``}`;
...
const createCards = (quantity) => {
  ...
  for (let i = 0; i < quantity; i++) {
    cards += createCard(getRandomTask());
  }
...
const renderCards = (quantity) => {
  boardTasks.innerHTML = createCards(quantity);
};
